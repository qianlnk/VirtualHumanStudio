	...
	for mode, ids := range modIds {
		...
		case "digital_human":
			...
			for _, task := range tasks {
				...
				if task.AudioURL != "" {
					audioURL, _ = storages.Client.GetFileUrl(context.Background(), task.AudioURL)
				}
				if task.VideoURL != "" {
					videoURL, _ = storages.Client.GetFileUrl(context.Background(), task.VideoURL)
				}
				inputParams := []*models.InputParam{
					{Key: "audio_file", Alias: "音频文件", Type: "audio", Value: audioURL},
					{Key: "video_file", Alias: "视频文件", Type: "video", Value: videoURL},
				}
				...
				if task.ResultURL != "" {
					resultURL, _ = storages.Client.GetFileUrl(context.Background(), task.ResultURL)
				}
				outputParams := []*models.InputParam{
					{Key: "result_video", Alias: "合成视频", Type: "video", Value: resultURL},
				}

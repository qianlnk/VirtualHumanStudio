...
func UploadMaskToComfyUI(maskPath string, modelRef string) (string, error) {
	...
	file, err := storages.Client.OpenFile(context.Background(), maskPath)
	if err != nil {
...
func UploadImageToComfyUI(imagePath string) (string, error) {
	// 打开图片文件
	file, err := storages.Client.OpenFile(context.Background(), imagePath)
	if err != nil {
...
func DownloadComfyUIResult(data map[string]interface{}, ext string, savePath string, nodeID string) error {
	...
	err = storages.Client.SaveFile(context.Background(), savePath, resp.Body)
	if err != nil {

...
func convertComfyUITaskToResponse(task models.ComfyUIWorkflowTask) InspirationTaskResponse {
	...
	if task.TaskType == "accessory" {
		...
		resultURL, _ = storages.Client.GetFileUrl(context.Background(), accessory.ResultImage)

		// 设置输入参数
		var itemImageURL, modelImageURL, maskImageURL string
		if accessory.ItemImage != "" {
			itemImageURL, _ = storages.Client.GetFileUrl(context.Background(), accessory.ItemImage)
		}
		if accessory.ModelImage != "" {
			modelImageURL, _ = storages.Client.GetFileUrl(context.Background(), accessory.ModelImage)
		}
		if accessory.MaskImage != "" {
			maskImageURL, _ = storages.Client.GetFileUrl(context.Background(), accessory.MaskImage)
		}
		inputParams = []InspirationParam{
			{Key: "item_image", Label: "物品图", Type: "image", Value: itemImageURL},
			{Key: "model_image", Label: "模特图", Type: "image", Value: modelImageURL},
			{Key: "mask_image", Label: "蒙版图", Type: "mask", Value: maskImageURL},
		}

		// 设置输出参数
		outputParams = []InspirationParam{
			{Key: "result_image", Label: "结果图", Type: "image", Value: resultURL},
		}
	} else {
		// 查询工作流任务获取更多信息
		...
		if db.DB.Where("id = ?", task.ID).First(&workflowTask).Error == nil {
			...
			if workflowTask.InputParams != "" {
				...
				for _, param := range params {
					...
					if param.Type == "image" || param.Type == "mask" || param.Type == "video" || param.Type == "audio" {
						inputParam.Value, _ = storages.Client.GetFileUrl(context.Background(), param.Value)
					}
			...
			if workflowTask.OutputParams != "" {
				...
				for i, param := range outputParams {
					if param.Type == "image" || param.Type == "mask" || param.Type == "video" || param.Type == "audio" {
						resultURL, _ = storages.Client.GetFileUrl(context.Background(), param.Value)
						outputParams[i].Value = resultURL
					}
				}
			}
		}
...
func convertDigitalHumanToResponse(dh models.DigitalHuman) InspirationTaskResponse {
	...
	if dh.AudioURL != "" {
		audioURL, _ = storages.Client.GetFileUrl(context.Background(), dh.AudioURL)
	}
	if dh.VideoURL != "" {
		videoURL, _ = storages.Client.GetFileUrl(context.Background(), dh.VideoURL)
	}

	// 适配数字人任务为与图像处理任务相同的格式
	inputParams := []InspirationParam{
		{Key: "audio_file", Label: "音频文件", Type: "audio", Value: audioURL},
		{Key: "video_file", Label: "视频文件", Type: "video", Value: videoURL},
	}
	...
	var resultURL string
	if dh.ResultURL != "" {
		resultURL, _ = storages.Client.GetFileUrl(context.Background(), dh.ResultURL)
	}
	outputParams := []InspirationParam{
		{Key: "result_video", Label: "合成视频", Type: "video", Value: resultURL},
	}

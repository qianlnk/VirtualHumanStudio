...
func processTxt2ImgTask(ctx context.Context, taskID uint) error {
	...
	for _, param := range moduleConfig.OutputParams {
		...
		err = storages.Client.SaveFile(context.Background(), filePath, resp.Body)
		if err != nil {
			task.Status = "failed"
			task.ErrorMsg = fmt.Sprintf("保存文件失败: %v", err)
			db.DB.Save(&task)
			return err
		}
...
func processImageTask(ctx context.Context, taskID uint) error {
	...
	for _, param := range inputParams {
		...
		if param.Type == models.ParamTypeImage {
			...
			fileRef, err := UploadMaskToComfyUI(param.Value, replaceVariables["{{"+strings.TrimSuffix(param.Key, "Mask")+"}}"])
			if err != nil {
				task.Status = "failed"
				task.ErrorMsg = fmt.Sprintf("文件上传失败(%s): %v", param.Key, err)
				db.DB.Save(&task)
	...
	for {
		...
		if status == "completed" && data != nil {
			...
			for _, param := range moduleConfig.OutputParams {
				...
				err := DownloadComfyUIResult(data, ext, filePath, param.Key)
				if err != nil {
					task.Status = "failed"
					task.ErrorMsg = fmt.Sprintf("下载结果图片失败: %v", err)
					db.DB.Save(&task)

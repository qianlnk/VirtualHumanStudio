<template>
  <div class="tts-container" :class="{'input-focused': isInputFocused}">
    ...
    <div v-loading="loading" class="task-list mobile-card-view" v-show="!isCardView">
      ...
      <el-table v-else :data="tasks" style="width: 100%" class="responsive-table">
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template slot-scope="scope">
            <el-tag :type="getStatusType(scope.row.status)" size="small">{{ getStatusText(scope.row.status, scope.row) }}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog 
      ...
      <el-form :model="form" :rules="rules" ref="form" label-width="100px" class="tts-form" :label-position="isMobile ? 'top' : 'left'">
...
export default {
  ...
  methods: {
    ...
    // 加载更多数据
    loadMoreTasks() {
      if (this.loadingMore || !this.hasMoreData || this.dialogVisible || this.pauseScrollListening) {
        this.debug('跳过加载更多:', '加载中=', this.loadingMore, '没有更多数据=', !this.hasMoreData, '对话框打开=', this.dialogVisible, '暂停滚动监听=', this.pauseScrollListening)
        return
      }
    ...
    handleWindowScroll() {
      // 如果滚动监听被暂停，则不处理
      if (this.pauseScrollListening || this.dialogVisible) {
        return;
      }
    ...
    submitForm() {
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          // 确保在移动端上，即使提交失败也能正确清理状态
          if (this.isMobile && this.dialogVisible) {
            this.dialogVisible = false
          }
        }
      })
    ...
    copyText(text) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text)
          ...
          .catch(err => {
            console.error('Clipboard API复制失败，尝试备选方案:', err)
            this.fallbackCopyText(text)
          })
      } else {
        // 对于不支持Clipboard API的浏览器，使用备选方案
        this.fallbackCopyText(text)
      }
    },
    ...
    fallbackCopyText(text) {
      try {
        ...
        span.style.MozUserSelect = 'all'
        span.style.msUserSelect = 'all'
    ...
    setupIntersectionObserver() {
      ...
      this.$nextTick(() => {
        ...
        this.observer = new IntersectionObserver((entries) => {
          ...
          // 添加对话框状态检查
          if (entry.isIntersecting && !this.loadingMore && this.hasMoreData && !this.dialogVisible && !this.pauseScrollListening) {
...
@media screen and (max-width: 768px) {
  ...
  .tts-form {
    ...
    padding-bottom: 70px !important; /* 为底部按钮留出空间 */
  }
